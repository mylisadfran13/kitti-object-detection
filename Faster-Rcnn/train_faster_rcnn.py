...
if __name__ == "__main__":
    ...
    print("модель сохранена как faster_rcnn_model.pth")
    ...
    print(f"всего объектов в выборке: {total_objects}")
    print(f"найдено объектов (conf > 0.5): {detected}")
    if total_objects > 0:
        print(f"примерный Recall: {detected / total_objects:.4f}")
    
    plt.figure(figsize=(10, 5))
    plt.plot(range(1, 11), losses, marker='o', color='#F18F01')
    plt.xlabel('Эпоха')
    plt.ylabel('Loss')
    plt.title('Faster R-CNN Training Loss')
    plt.grid(True)
    plt.savefig('faster_rcnn_loss.png', dpi=150)